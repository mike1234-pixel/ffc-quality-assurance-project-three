  ...
  */
  ...
  suite('Routing tests', function() {
    ...
    suite('POST /api/books with title => create book object/expect book object', function() {
      ...
      chai.request(server)
        .post('/api/books').type('form').send({ title: "test-title"})
        .end(function(err, res) {
          assert.equal(res.status, 200);
          ...
          id = res.body._id; // will use this in later test to request book using id
    ...
    suite('POST /api/books/[id] => add comment/expect book object with id', function(){
      ...
      chai.request(server)
        .post('/api/books/' + id).type('form').send({ comment: "test-comment"})
        .end(function(err, res) {
          assert.equal(res.status, 200);
          assert.isObject(res.body, 'response should be an object')
          assert.property(res.body, 'commentcount', 'Book should contain commentcount');
